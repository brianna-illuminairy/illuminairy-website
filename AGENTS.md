...
5. **Deploy path** — **One repo, one Vercel build** (Plan Builder, `/satplan`, LP, APIs ship together). **Git → GitHub → Vercel auto-deploy** on push to `main` only. Ship: `npm run agent:verify` → commit → `git push origin main` (or `npm run release`). **Never** use Vercel CLI for deploy or production env (`deploy:cli`, `vercel deploy`, `vercel env *`, `env:sync`) unless Brianna explicitly asks. Env changes: Vercel dashboard → push `main` to rebuild. Guides: [`docs/deploy.md`](docs/deploy.md), [`docs/production-surfaces.md`](docs/production-surfaces.md).
