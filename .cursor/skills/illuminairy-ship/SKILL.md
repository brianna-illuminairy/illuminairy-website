...
4. **Deploy:** commit on `main`, then `git push origin main` or `npm run release` (GitHub → Vercel auto-deploy). **Never** Vercel CLI deploy or production env (`deploy:cli`, `vercel deploy`, `vercel env *`, `env:sync`) unless the user explicitly asks. See `docs/deploy.md`.
