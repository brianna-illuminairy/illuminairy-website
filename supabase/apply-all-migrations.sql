-- Illuminairy CRM: leads, clients, students, enrollments, touch_events
...
create table enrollments (
  ...
  lead_id uuid references leads (id) on delete set null,
  program text not null default 'sat-accelerator',
  program_label text,
...
-- Fast-query visitor attribution/audience columns
...
create index if not exists visitors_first_utm_content_idx on visitors (first_utm_content);
create index if not exists visitors_first_hero_hook_idx on visitors (first_hero_hook);
create index if not exists visitors_quiz_who_idx on visitors (quiz_who);
