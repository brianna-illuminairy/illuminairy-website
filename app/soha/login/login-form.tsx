...
import { trackSohaPortalLogin } from "@/lib/soha-portal-analytics";
...
export default function SohaLoginForm() {
  const router = useRouter();
  const params = useSearchParams();
  const next = params.get("next") ?? "/soha/diagnostic";
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/soha/login", {
        method: "POST",
