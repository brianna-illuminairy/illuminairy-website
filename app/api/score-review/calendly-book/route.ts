...
import { bookScoreReviewInvitee } from "@/lib/calendly/book-score-review";
import {
...
export async function POST(request: Request) {
  ...
  const scoreReviewUrl = site.scoreReviewCalendlyUrl;

  try {
    const [stillOpen, meta] = await Promise.all([
      isSlotStillAvailable(token, startTime, scoreReviewUrl),
      fetchEventTypeBookingMeta(token, scoreReviewUrl),
    ]);
    if (!stillOpen) {
      await recordBookingError({
    ...
    const result = await bookScoreReviewInvitee(
      token,
      {
        startTime,
        parentName,
    ...
    if (supabase) {
      const { data: lead } = await supabase
        .from("leads")
        .select("id, visitor_id, parent_first, parent_last, parent_phone, fbclid, meta_fbp, meta_fbc, meta_fbc_ts, meta_client_ip, meta_client_user_agent")
        .eq("parent_email", email)
        .maybeSingle();
      ...
      if (lead) {
        ...
        const callRow = {
          ...
          call_type: "score_review" as const,
        };
        ...
        await appendTouchEvent({
          ...
          lead_id: lead.id,
          event_type: "score_review_booked",
          source: "server",
          attribution,
          payload: {
            calendly_uri: result.inviteeUri,
            score_review_at: result.startTime,
            funnel: SCORE_REVIEW_FUNNEL_KEY,
        ...
        void trackKlaviyoEvent(email, KlaviyoEvents.scoreReviewBooked, {
          funnel: SCORE_REVIEW_FUNNEL_ID,
          score_review_at: result.startTime,
        });

        const scheduleEventId = makeMetaEventId("schedule", result.inviteeUri.split("/").pop() ?? lead.id);
        void sendMetaCapiEvent(
          "Schedule",
          scheduleEventId,
          {
            email,
            phone: lead.parent_phone ?? parentPhone,
            firstName: lead.parent_first ?? parentName.split(/\s+/)[0],
            lastName: lead.parent_last ?? "",
            externalId: lead.id,
            fbp: lead.meta_fbp ?? undefined,
            fbc: lead.meta_fbc ?? undefined,
            fbcTs: lead.meta_fbc_ts ?? undefined,
    ...
    if (err instanceof CalendlyBookError) {
      console.error("[score-review/calendly-book]", err.code, err.message);
      await recordBookingError({
        errorCode: err.code,
    ...
    console.error("[score-review/calendly-book]", message);
    await recordBookingError({
      errorCode: code,
