...
const AVAILABILITY_EMPTY =
  "No lesson times are open right now. Check back tomorrow or email support@illuminairy.com.";
...
export async function GET(request: Request) {
  ...
  try {
    const days = await fetchFunnelSchedulerDays(token, site.freeLessonCalendlyUrl, { fresh });
    if (days.length === 0) {
    ...
    console.error("[funnel-b/calendly-availability]", message);
    return NextResponse.json(
      {
        ok: false,
