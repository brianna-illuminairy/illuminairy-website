...
/**
 * Body for PostHog step `achievability` (goal score achievability rating, pre-name).
 * NOT the plan reveal — use `QFV1Projection` for step `v1`.
 *
 * @param {{
 *   plan: import('@/lib/quiz-funnel/goal-achievability-screen').GoalAchievabilityScreenModel,
 ...
 * }} props
 */
export function GoalAchievabilityContent({ plan, title, introNote, q2, answers, onEditAnswer }) {
  const assessment = resolveGoalAchievabilityForDisplay(plan, { q2: q2 ?? plan.q2, qWho: answers?.qWho });
  return (
