/**
 * View model for the **goal achievability rating** screen (PostHog step `achievability`).
 *
 * NOT the plan reveal — that is step `v1` / `QFV1Projection` (`lib/quiz-funnel/v1-projection.ts`).
 * Historical module name `plan-reveal` was misleading; import this file instead.
 */
import {
...
function everythingVsFew(qWho?: string): string {
  const { possessive } = quizSubjectVoice(qWho);
  return `everything on the SAT instead of the few skills that actually move ${possessive} score`;
}
...
function q7WhyFailed(qWho?: string): Record<string, string> {
  const self = isQuizSelfTaker(qWho);
  return {
    khan: self
      ? "Khan walks through the whole course, not the 5–6 skills the Skill Diagnostic will show matter most for you."
      : "Khan walks through the whole course, not the 5–6 skills the Skill Diagnostic will show matter most for them.",
    group: self
      ? "The class taught every topic at the same pace. Nobody sorted out which skills you were actually missing."
      : "The class taught every topic at the same pace. Nobody sorted out which skills your student was actually missing.",
    online: self
      ? "Everyone got the same syllabus. No one ranked what you should work on first."
      : "Everyone got the same syllabus. No one ranked what your student should work on first.",
    app: "The app kept giving practice questions. It never named which skills to fix first.",
    book: self
      ? "The book covers the whole test. It doesn't tell you where to start."
      : "The book covers the whole test. It doesn't tell you where to start for your student.",
    nothing: self
      ? "Without the Skill Diagnostic, you had to guess where to start and spent time on the wrong topics."
...
function whyLastPrepFailed(q7: unknown, qWho?: string): string {
  const ids = normalizeQ7(q7);
  const whyFailed = q7WhyFailed(qWho);
  const key = Q7_PRIORITY.find((id) => ids.includes(id));
  if (key && whyFailed[key]) return whyFailed[key];
  const named = selectedPrepLabels(ids);
  const spread = everythingVsFew(qWho);
  if (named.length) {
    return `${named.join(" and ")} covered ${spread}.`;
  }
  return `Past prep spread time across ${spread}.`;
}
...
export function buildInputGroups(answers: QuizAnswersLike): PlanRevealInputGroup[] {
  ...
  student.push({
    label: isQuizSelfTaker(answers.qWho) ? "Your goal" : "Their goal",
    value: q2Label(q2),
  });
...
function buildHeardSummary(answers: QuizAnswersLike, path: ScorePathOutput): string {
    ...
    q4 === "na"
      ...
      : hasKnownStartingScore(q4)
        ...
        : self
          ...
          : "They're still pinning down their starting score";
  ...
  let sentence = self
    ...
    : `${startPhrase}, they ${targetPhrase}, ${datePhrase}.`;
  if (prep.length) {
    sentence += self
...
export type GoalAchievabilityScreenLever = { rank: number; name: string; pts: number };

/** @deprecated Use GoalAchievabilityScreenLever */
export type PlanRevealLever = GoalAchievabilityScreenLever;

export type GoalAchievabilityScreenModel = {
  q2?: string;
  ...
  topLevers: GoalAchievabilityScreenLever[];
  leversNote: string;
...
/** @deprecated Use GoalAchievabilityScreenModel */
export type PlanRevealModel = GoalAchievabilityScreenModel;

export function buildGoalAchievabilityScreenModel(
  answers: QuizAnswersLike
): GoalAchievabilityScreenModel {
  ...
  return {
    ...
    topLevers: levers,
    leversNote,
    whyLastTimeFailed: whyLastPrepFailed(answers.q7, answers.qWho),
...
/** @deprecated Use buildGoalAchievabilityScreenModel — builds achievability step, not plan reveal (`v1`). */
export const buildPlanReveal = buildGoalAchievabilityScreenModel;
