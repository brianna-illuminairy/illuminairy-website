/**
 * Book via Calendly Scheduling API — parent stays on our s5 UI (no popup form).
...
export type BookStrategyCallInput = {
  startTime: string;
  parentName: string;
  parentEmail: string;
  parentPhone?: string;
...
export type BookStrategyCallResult = {
  inviteeUri: string;
  startTime: string;
};

/** Create invitee at `startTime` using data already collected on s5. */
export async function bookStrategyCallInvitee(
  token: string,
  input: BookStrategyCallInput,
  options?: { publicUrl?: string; meta?: EventTypeBookingMeta }
): Promise<BookStrategyCallResult> {
  const publicUrl = options?.publicUrl ?? site.calendlyUrl;
  const meta =
