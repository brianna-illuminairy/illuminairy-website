...
export const SHERMEEN_OVERVIEW_INTRO = [
  ...
  "We will help her build this foundation by teaching the question types, how to identify what each question is asking, and the best method to solve it. We go through examples every session until she has the type, the ask, and the method memorized. When that step is automatic, she can get started on each question right away instead of losing time figuring out how to set the problem up or reading through every answer choice before she knows what she is looking for. That saves time across the module and lowers the risk of running out of time at the end. There are more than 20 question types across Reading and Writing and Math, so this takes repetition, but it is the same process in each section.",
...
export const SHERMEEN_RW_PLAN_NOTE =
  "Because the misses are spread out, we still work one skill at a time in ranked order. Each session we teach the question type, how to identify what it is asking, and the best method to use, then walk through examples until she has it memorized. The goal is for that identification to become automatic so she can start solving immediately instead of burning time on setup or re-reading every choice before she knows what the question wants. Timed practice on that skill follows until she hits 95%+ accuracy on medium questions before we move to the next.";
...
export const SHERMEEN_MATH_INTRO = [
  ...
  "We build that foundation in tutoring by teaching the question types, how to identify what is being asked, and the best method to solve. We go through examples every session until she has them memorized. When type and method are automatic, she can start the math immediately on each question instead of spending the first minute deciding how to set the problem up. That protects time across the module and reduces the risk of rushing or guessing at the end. Desmos helps only after that setup is clear.",
];
...
export const SHERMEEN_TIMING_INTRO = [
  ...
  "Much of the time story on this test connects to question-type and method recognition. When that step is not automatic yet, students often spend long stretches on setup or reading every answer choice and still miss, or they rush a guess at the end because earlier questions took too long. Making type identification and the solve method automatic is one of the main ways we protect pacing: she can get started on each question quickly instead of figuring out how to approach it from scratch.",
];
