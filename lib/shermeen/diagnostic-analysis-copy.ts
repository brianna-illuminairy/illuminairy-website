...
export const SHERMEEN_OVERVIEW_INTRO = [
  ...
  "Shermeen did better on hard Reading and Writing questions than on easy ones there, scoring 70% on hard and 38% on easy. Based on experience, that pattern is typical of a student who has not been taught three things yet: what question types exist on the SAT, how to identify what a given question is asking for, and the best memorized method to use once she has named the type. When those three steps are not automatic, easy questions can miss more often than hard ones, because easy questions usually test a specific rule or question type rather than reading stamina or passage difficulty.",
  "That is fixable, but it takes time and repetition. On many of these wrong answers, I suspect she already knows the raw math or grammar rule involved. What she is missing is the step before that: naming what the question is asking, and knowing which formula or approach to apply first. That is exactly what we teach through experience, by completing problems over and over until recognition and method become automatic. There are more than 20 question types across Reading and Writing and Math, and tutoring runs until she can name the type, name what it is asking, and run the right method without hesitating.",
...
export const SHERMEEN_ADAPTIVE_RW =
  "On Reading and Writing, Shermeen got 21 of 27 correct in Module 1 (77.8%), enough that Module 2 used the harder question set. In Module 2 she got 13 of 27 correct. Most of her Reading and Writing wrong answers were in Module 2, but the root issue showed up earlier: six of her seven Module 1 wrong answers were easy or medium questions on grammar, transitions, evidence, and rhetorical synthesis.";
...
export const SHERMEEN_RW_INTRO_LEAD =
  "Shermeen missed 20 Reading and Writing questions across both modules, spread across transitions, command of evidence, form and structure, boundaries, text structure, central ideas, inferences, rhetorical synthesis, and words in context. She scored 70% on hard questions and 38% on easy in this section. That fits the overview pattern: she has not yet been taught to name the question type, identify what that question is asking for, and apply the memorized method for that type before she reads the answer choices. On many of these, I do not think the gap is that she cannot read the passage. She is unsure what the question is asking her to do with it. In Module 1 she got hard craft-and-structure and central-ideas questions right while missing easy transition, evidence, and rhetorical synthesis questions. On most wrong answers she picked something that sounded plausible in the passage but did not match what that question type was asking for.";

export const SHERMEEN_RW_PLAN_NOTE =
  "Because the misses are spread out, we still attack one skill at a time in order of point impact, not a single crash course on everything at once. On each skill the work is the same: name the question type, name what it is asking, apply the memorized method, then check the answer. She completes timed practice on that skill until she hits 95%+ accuracy on medium questions before we move to the next.";
...
export const SHERMEEN_MATH_INTRO = [
  ...
  "When I reviewed each wrong answer in Mentomind, a pattern showed up underneath the topic labels. On many of them, I suspect she already knows the raw math for how to solve once the problem is set up. What she is unsure about is what the question is asking, or which formula or approach to apply first. Module 1 Question 14 is a clear example: she found the x-intercept (h = 4) but did not finish with h + k. Module 2 Question 20 she answered in 19 seconds on a factor-theorem problem, which suggests she never named the setup step at all. Several other misses (factoring, circle tangency, arc length) follow the same shape: the algebra is teachable; the missing piece is recognizing the question type and reaching for the right method immediately.",
  ...
  "That recognition work is exactly what we build through experience: completing problems over and over until she can name the type, name what it is asking, pick the formula or approach, and then do the math she already mostly knows. Desmos helps only after that setup is clear.",
];
...
export const SHERMEEN_MATH_PLAN_NOTE =
  "Given her Math base on easy questions (92% correct), I would attack one skill block at a time: factoring and the factor theorem first, then circles and arc length, then linear word problems and finish-the-question habits. The goal on each block is not just reviewing math she has seen in school. It is drilling the moment before the math: name the question type, name what it is asking, pick the approach, then execute. After each lesson she would complete timed practice until she hits 95%+ accuracy on medium questions in that skill before we move to the next.";
...
export const SHERMEEN_MATH_QUESTION_TYPES =
  "A large part of SAT Math is recognizing what the question is asking before you calculate. The SAT reuses the same question types, and the same topic can look different depending on the wording. Once you know the type, the formula or method usually follows. In tutoring we learn what each type looks like: two points and \"find the equation\" means write a line equation; a grid-in with a system means solve and enter the exact value; a circle question with a diagram means pull the radius or arc relationship first.";
