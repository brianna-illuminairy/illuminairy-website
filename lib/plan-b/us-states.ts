/** US states + DC — codes, slugs, display labels. */
...
const SLUG_OVERRIDES: Partial<Record<UsStateCode, string>> = {
  DC: "dc",
};

export function stateLabel(code: UsStateCode): string {
...
export function stateCodeFromSlug(slug: string): UsStateCode | null {
  const normalized = slug.trim().toLowerCase();
  for (const code of US_STATE_CODES) {
    if (stateSlugFromCode(code) === normalized) return code;
  }
  return null;
}
