...
- **Public:** `NEXT_PUBLIC_CALENDLY_URL` → `PUBLIC_SAT_STRATEGY_CALL_CALENDLY_URL` in [`lib/site.ts`](../../lib/site.ts) (`sat-planning-session`, one-on-one). Embedded on `/contact#schedule` and Plan Builder s5; must stay in sync with Vercel env (`npm run verify:calendly-parity`; set env in Vercel dashboard, then push `main`).
