...
const LESSONS = [
  {
    href: "/danielle/week-3/lesson-1",
    label: "Lesson 1 · Tue Jun 23",
...
export function Week3LessonNav() {
  const pathname = usePathname() ?? "";
  const router = useRouter();

  if (!pathname.startsWith("/danielle/week-3")) {
    return null;
  }
    ...
    <div className="danielle-portal__week1-subnav">
      <div className="danielle-portal__week1-subnav-inner">
        <label className="danielle-portal__week1-picker">
          <span className="danielle-portal__week1-picker-label">Week 3</span>
          <select
            className="danielle-portal__week1-select"
            value={active}
            onChange={(e) => router.push(e.target.value)}
            aria-label="Choose Week 3 section"
          >
            {LESSONS.map((lesson) => (
        ...
        <div className="danielle-portal__week1-tabs" aria-label="Week 3 sections">
          {LESSONS.map((lesson) => {
