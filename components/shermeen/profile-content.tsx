...
import {
  ...
  SHERMEEN_PROFILE,
  type ProfilePerson,
} from "@/lib/shermeen/profile-data";
...
export function ShermeenProfileContent() {
  return (
    <div className="skye-profile">
      <header className="aurora-portal__page-head">
        ...
        <h1 className="aurora-portal__title">Shermeen&apos;s profile</h1>
      </header>

      <section className="skye-profile__section">
        <h2>Family</h2>
        <div className="skye-profile__people">
          <PersonCard person={SHERMEEN_PROFILE.parent} />
          <PersonCard person={SHERMEEN_PROFILE.student} />
        </div>
      </section>
      ...
      <section className="skye-profile__section">
        <h2>About Shermeen</h2>
        <ul className="skye-profile__list">
          {SHERMEEN_PROFILE.about.map((line) => (
            <li key={line}>{line}</li>
          ))}
        </ul>
      </section>
      ...
      <section className="skye-profile__section">
        ...
        <ul className="skye-profile__list">
          {SHERMEEN_PROFILE.testingNotes.map((line) => (
            <li key={line}>{line}</li>
          ))}
        </ul>
      </section>
      ...
      <section className="skye-profile__section">
        ...
        <dl className="skye-profile__timeline">
          {SHERMEEN_PROFILE.timeline.map((row) => (
            <div key={row.label}>
              <dt>{row.label}</dt>
              <dd>{row.value}</dd>
            </div>
      ...
      <section className="skye-profile__section">
        ...
        <ul className="skye-profile__list">
          {SHERMEEN_PROFILE.workingOn.map((line) => (
            <li key={line}>{line}</li>
          ))}
        </ul>
        <p className="skye-profile__links">
          See the <Link href="/shermeen/diagnostic">diagnostic analysis</Link> for question-level detail
          from the June 23 Skill Diagnostic.
        </p>
      </section>
    </div>
  );
