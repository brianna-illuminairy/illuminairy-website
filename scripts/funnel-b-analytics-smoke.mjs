...
PostHog (filter plan_builder_funnel = b OR funnel_id = plan_builder_b, URL /plan-b):
  ...
  [ ] lab_lead_submitted on b-book lead save
  [ ] lab_lesson_booked + quiz_booking_confirmed on slot confirm
  [ ] lab_lesson_link_shared after share step

Meta Test Events:
  [ ] Lead — pixel + CAPI same event_id on lead submit (/api/funnel-b/lead)
  [ ] Schedule — pixel + CAPI on free lesson book (event_id schedule_<inviteeId>)
